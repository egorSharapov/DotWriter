...
protected:
  virtual void PrintName(std::ostream &out) { out << AttributeType::ToString(_type); }
...
public:
   ...
   */
  CustomAttribute(const std::string &type, const std::string &value) : _type(type), _value(value) {}
...
class HTMLLabelAttribute : public CustomAttribute {
public:
  HTMLLabelAttribute(std::string type, std::string value)
      : CustomAttribute(std::move(type), std::move(value)) {}
 ...
 */
template <typename T, typename F> class EnumAttribute : public StandardAttribute {
private:
  T _value;
...
public:
  EnumAttribute(AttributeType::e type, T value) : StandardAttribute(type), _value(value) {}
 ...
 * T is the enum type, and F is the struct that houses its ToString() method.
 */
template <typename T, typename F> class EnumListAttribute : public StandardAttribute {
private:
...
public:
  EnumListAttribute(AttributeType::e type, T value) : StandardAttribute(type) { AddValue(value); }

  EnumListAttribute(AttributeType::e type, const std::vector<T> &values) : StandardAttribute(type) {
    // Copies contents of the input vector.
    _values = values;
  }

  void RemoveValue(T value) {
    typename std::vector<T>::iterator loc = std::find(_values.begin(), _values.end(), value);
...
protected:
  virtual void PrintValue(std::ostream &out) { out << (_value ? "true" : "false"); }

public:
  BoolAttribute(AttributeType::e type, bool value) : StandardAttribute(type), _value(value) {}
 ...
 */
template <typename T> class SimpleAttribute : public StandardAttribute {
private:
  T _value;
...
public:
  SimpleAttribute(AttributeType::e type, T value) : StandardAttribute(type), _value(value) {}
 ...
 */
template <typename T> class SimpleListAttribute : public StandardAttribute {
private:
  std::vector<T> _values;
...
public:
  SimpleListAttribute(AttributeType::e type, T value) : StandardAttribute(type) { AddValue(value); }
...
public:
  AddDoubleAttribute(AttributeType::e type, double value) : SimpleAttribute<double>(type, value) {}
};
...
public:
  AddPointAttribute(AttributeType::e type, double x, double y) : PointAttribute(type, x, y) {}
};
...
public:
  PointListAttribute(AttributeType::e type, double x, double y) : StandardAttribute(type) {
    AddPoint(x, y);
  }

  void AddPoint(double x, double y) { _values.push_back(std::pair<double, double>(x, y)); }
};
